...
class LocalPoolPointnetMerge(nn.Module):
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max',
                 ...
                 plane_resolution=None, grid_resolution=None, plane_type='xz', padding=0.1, n_blocks=5, pos_encoding = False):
        super().__init__()
        self.c_dim = c_dim
    ...
    def generate_grid_features(self, p, c):
        ...
        if self.unet3d is not None:
            #fea_grid, latent = self.unet3d(fea_grid)
            latent = self.unet3d(fea_grid)

        return latent

    def pool_local(self, index_key, c):
        bs, fea_dim = c.size(0), c.size(2)

        fea = self.scatter(c.permute(0, 2, 1), index_key, dim_size=self.reso_grid**3)

        if self.scatter == scatter_max:
            fea = fea[0]
        # gather feature back to points
        fea = fea.gather(dim=2, index=index_key.expand(-1, fea_dim, -1))
    ...
    def forward(self, p):
        ...
        if 'grid' in self.plane_type:
            # First scan
            coord['grid_1'] = normalize_3d_coordinate(p1.clone(), padding=self.padding)
            index['grid_1'] = coordinate2index(coord['grid_1'], self.reso_grid, coord_type='3d')

            # Second scan
            coord['grid_2'] = normalize_3d_coordinate(p2.clone(), padding=self.padding)
            index['grid_2'] = coordinate2index(coord['grid_2'], self.reso_grid, coord_type='3d')

            # First + Second
            coord['grid_12'] = normalize_3d_coordinate(p12.clone(), padding=self.padding)
            index['grid_12'] = coordinate2index(coord['grid_12'], self.reso_grid, coord_type='3d')
...
class PatchLocalPoolPointnet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if T == 800000:
            fea['grid'] = torch.ones(batch_size, 32, 88, 88, 88).to('cuda') * -1000000
